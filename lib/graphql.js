import { GET_ALL_POST_QUERY, GET_ALL_POST_WITH_CONTENT_QUERY, GET_CATEGORY_QUERY, GET_CATEGORY_SITEMAP_QUERY, GET_POST_SITEMAP_QUERY, GET_SINGLE_POST_QUERY, GET_TAG_QUERY, GET_TAG_SITEMAP_QUERY, GET_WEB_STORIES_BY_SLUG_QUERY, GET_WEB_STORY_SITEMAP_QUERY, GET_ALL_WEB_STORIES } from "constant/query";
import config from "@config/config.json";
const { getAllPostLog, getAllPostContentLog,
  getPostLog,
  getCategoryLog,
  getTagLog,getWebStoryLog } = config.logging;
//Staging URL
...
//Function to get data of web stories for sitemap with pagination
export async function getWebStoryForSiteMap(perPage, after = null) {
  try {
    const data = await fetchAPI(
      GET_WEB_STORY_SITEMAP_QUERY,
      {
        variables: { perPage, after },
      }
    );
